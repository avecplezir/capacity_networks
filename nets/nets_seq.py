...
class QNetworkLSTMNamedTupleEval(nn.Module):
    def __init__(self, env, args):
        super().__init__()
        self.args = args
        self.q_value_dim = env.single_action_space.n
        ...
        self.lstm = nn.LSTM(512, 128, batch_first=False, num_layers=1)
        self.predictor = nn.Linear(128, self.q_value_dim)
        self.predictor_eval = nn.Linear(128, self.q_value_dim)
        self.ActorOut = namedtuple(
            "ActorOut",
            [
                "q_values",
    ...
    def forward(self, data):
        ...
        out = self.ActorOut(q_values=q_values, q_values_eval=q_values_eval, h=h, c=c)
        return out

    def init_net_hiddens(self):
        q_values = -torch.ones(self.args.num_envs, self.q_value_dim).to(self.args.device)
        q_values_eval = -torch.ones(self.args.num_envs, self.q_value_dim).to(self.args.device)
        h, c = \
            torch.zeros(self.args.num_envs, self.lstm.num_layers, self.lstm.hidden_size).to(self.args.device), \
            torch.zeros(self.args.num_envs, self.lstm.num_layers, self.lstm.hidden_size).to(self.args.device),
        return self.ActorOut(q_values=q_values, q_values_eval=q_values_eval, h=h, c=c)
...
class QNetworkMinAtarLSTMEval(nn.Module):
    def __init__(self, env, args):
        super().__init__()
        self.args = args
        self.encoder = nn.Sequential(
        ...
        self.lstm = nn.LSTM(256, 128, batch_first=False, num_layers=1)
        self.predictor = nn.Linear(128, env.single_action_space.n)
        self.predictor_eval = nn.Linear(128, env.single_action_space.n)
    ...
    def forward(self, x, hiddens):
        ...
        q_values = self.predictor(out)
        q_values_eval = self.predictor_eval(out)
        return q_values, q_values_eval, {'h': h, 'c': c}
